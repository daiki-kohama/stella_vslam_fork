...
public:
    lightglue(const float lowe_ratio, const bool check_orientation, feature::lg_matcher* lg_matcher)
        : base(lowe_ratio, check_orientation), lg_matcher_(lg_matcher) {}
    ...
    unsigned int match_frame_and_frame(data::frame& frm_1, data::frame& frm_2, std::vector<cv::Point2f>& prev_matched_pts,
                                       std::vector<int>& matched_indices_2_in_frm_, std::vector<float>& matched_scores_2_in_frm_1);

    unsigned int match_current_and_last_frames(data::frame& curr_frm, const data::frame& last_frm, const float margin, std::vector<float>& matched_scores_in_cur);
    ...
    unsigned int detect_duplication(const std::shared_ptr<data::keyframe>& keyfrm,
                                    ...
                                    bool do_reprojection_matching = false);
...
private:
    ...
    using keyfrm_dl_keypts_t = std::map<std::shared_ptr<data::keyframe>, std::unordered_set<unsigned int>, id_less<std::shared_ptr<data::keyframe>>>;
